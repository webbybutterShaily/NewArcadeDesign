...
const StackForm = (props) => {
    ...
    async function getStakedAmount() {
        ...
        const res = await request.get(`http://api.private.arcade2earn.io/api/demo/mission/staked/${props.data.mission_mint}/RCade47ZKErNcQB1CgkpEZUEmyfsqi2qh21mSCWASgm`)
        console.log('staked: ', res.data)
        setStakedAccount(res.data)
    }

    async function stakeAmount() {
        const res = await request.post(`http://api.private.arcade2earn.io/api/demo/mission/stake`, { mint: props.data.mission_mint, amount: amountToStake, address: "RCade47ZKErNcQB1CgkpEZUEmyfsqi2qh21mSCWASgm" })
    ...
    async function unstakeAmount() {
        const res = await request.post(`http://api.private.arcade2earn.io/api/demo/mission/unstake`, { mint: props.data.mission_mint, amount: amountToStake, address: "RCade47ZKErNcQB1CgkpEZUEmyfsqi2qh21mSCWASgm" })
        getAccountBalances()
        getStakedAmount();
        return res.data;
    }
                    ...
                    <div className='card shadow-widget stake-card'>
                        <div className='card-body'>
                            <div className="card-header-title">
                                <h3 className="card-title text-center text-gradient">Deposit</h3>
                            </div>
                            <form className='stake-form mx-xl-5'>
                                ...
                                <div className='text-label mb-1'>Amount to Deposit (Min : 0.01)</div>
                                ...
                                <div className='text-center mt-4'>
                                    <a href='#' onClick={() => nextStep('Staked')} className='btn btn-action px-3'>Deposit</a>
                                </div>
                            </form>
                        </div>
                    </div>
